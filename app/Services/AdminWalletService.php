...
class AdminWalletService
{
    ...
    public function deposit(User $user, float $amount, TransactionName $transactionName, array $meta = []): bool
    {
        ...
        try {
            DB::transaction(function () use ($user, $amount, $transactionName, $meta) {
                ...
                Log::debug('AdminWalletService::deposit - Before update', [
                    'user_id' => $user->id,
                ...
                Log::debug('AdminWalletService::deposit - After update', [
                    'user_id' => $user->id,
                    'user_name' => $user->user_name,
            ...
            Log::error('AdminWalletService::deposit failed', [
                'user_id' => $user->id,
                'amount' => $amount,
                'error' => $e->getMessage()
            ]);
    ...
    public function withdraw(User $user, float $amount, TransactionName $transactionName, array $meta = []): bool
    {
        ...
        try {
            DB::transaction(function () use ($user, $amount, $transactionName, $meta) {
                ...
                Log::debug('AdminWalletService::withdraw - Before update', [
                    'user_id' => $user->id,
                    'user_name' => $user->user_name,
                    'old_balance' => $oldBalance,
                    'amount' => $amount,
                ...
                Log::debug('AdminWalletService::withdraw - After update', [
                    'user_id' => $user->id,
                    'user_name' => $user->user_name,
                    'expected_new_balance' => $newBalance,
                    'actual_new_balance' => $actualNewBalance,
            ...
            Log::error('AdminWalletService::withdraw failed', [
                'user_id' => $user->id,
                'amount' => $amount,
                'error' => $e->getMessage()
            ]);
    ...
    public function transfer(User $from, User $to, float $amount, TransactionName $transactionName, array $meta = []): bool
    {
        ...
        try {
            ...
            Log::error('AdminWalletService::transfer failed', [
    ...
    /**
     * Force withdraw (allows negative balance) - bypasses balance checks
     */
    public function forceWithdraw(User $user, float $amount, TransactionName $transactionName, array $meta = []): bool
    {
        if ($amount <= 0) {
            return false;
        }
        ...
        try {
            DB::transaction(function () use ($user, $amount, $transactionName, $meta) {
                ...
                $oldBalance = (float) $lockedUser->balance;
                $newBalance = $oldBalance - $amount; // Force withdraw allows negative balance

                Log::debug('AdminWalletService::forceWithdraw - Before update', [
                    'user_id' => $user->id,
                    'user_name' => $user->user_name,
                    'old_balance' => $oldBalance,
                    'amount' => $amount,
                ...
                // Update balance directly in users table (allows negative)
                $lockedUser->update(['balance' => $newBalance]);
                ...
                Log::debug('AdminWalletService::forceWithdraw - After update', [
                    'user_id' => $user->id,
                    'user_name' => $user->user_name,
                    'expected_new_balance' => $newBalance,
                    'actual_new_balance' => $actualNewBalance,
                ...
                // Log transaction
                $this->logTransaction($user, $user, $amount, 'withdraw', $transactionName, $oldBalance, $newBalance, array_merge($meta, ['forced' => true]));
            });

            return true;
        } catch (\Exception $e) {
            Log::error('AdminWalletService::forceWithdraw failed', [
                'user_id' => $user->id,
                'amount' => $amount,
                'error' => $e->getMessage()
            ]);
    ...
    public function forceTransfer(User $from, User $to, float $amount, TransactionName $transactionName, array $meta = []): bool
    {
        ...
        try {
            ...
            Log::error('AdminWalletService::forceTransfer failed', [
                'from_user_id' => $from->id,
                'to_user_id' => $to->id,
                'amount' => $amount,
                'error' => $e->getMessage()
    ...
    public function initializeWallet(User $user, float $initialBalance = 0.0): bool
    {
        try {
            ...
            Log::error('AdminWalletService::initializeWallet failed', [
                'user_id' => $user->id,
                'error' => $e->getMessage()
            ]);
            return false;
