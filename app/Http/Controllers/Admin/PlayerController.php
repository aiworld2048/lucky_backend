...
class PlayerController extends Controller
{
    ...
    public function index()
    {
        ...
        // Step 1: Get all descendant player IDs under this owner/agent using recursive CTE
        ...
        ", [$startId, UserType::Player->value]))->pluck('id');
        ...
        $transferLogs = TransferLog::with(['fromUser', 'toUser'])
            ...
            ->get()
            // Group by player_id. A log might belong to two players in $playerIds if agent transfers to player
    ...
    public function create()
    {
        ...
        // Get the owner (Agent's parent)
        // For Agent role: agent_id points to Owner
        // For Owner role: owner will be null (Owner doesn't create players directly)
        $owner = null;
        if ($agent && $agent->agent_id) {
            $owner = User::where('id', $agent->agent_id)->where('type', UserType::Owner->value)->first();
        }
    ...
    {
        $agent = $this->getAgent() ?? Auth::user();
        $siteLink = "https://app.silentforestnature.xyz/";
    ...
    public function playerReportIndex($id)
    {
        ...
        // Get player's bets from place_bets table
        $reportDetail = DB::table('place_bets')
            ->select(
                'place_bets.*',
                DB::raw('CASE 
                    WHEN place_bets.currency = \'MMK2\' THEN place_bets.bet_amount * 1000 
                    ELSE place_bets.bet_amount 
                END as bet_amount_converted'),
                DB::raw('CASE 
                    WHEN place_bets.currency = \'MMK2\' THEN place_bets.prize_amount * 1000 
                    ELSE place_bets.prize_amount 
                END as prize_amount_converted')
            )
            ->where('place_bets.player_id', $id)
            ->whereBetween('place_bets.created_at', [$startDate.' 00:00:00', $endDate.' 23:59:59'])
            ->orderBy('place_bets.created_at', 'desc')
            ->paginate(20)
            ->appends([
        ...
        $total = [
            'total_bet_amt' => DB::table('place_bets')
                ->where('player_id', $id)
                ->whereBetween('created_at', [$startDate.' 00:00:00', $endDate.' 23:59:59'])
                ->selectRaw('SUM(CASE WHEN currency = \'MMK2\' THEN COALESCE(bet_amount, 0) * 1000 ELSE COALESCE(bet_amount, 0) END) as total')
                ->value('total') ?? 0,
            'total_payout_amt' => DB::table('place_bets')
                ->where('player_id', $id)
                ->whereBetween('created_at', [$startDate.' 00:00:00', $endDate.' 23:59:59'])
                ->selectRaw('SUM(CASE WHEN currency = \'MMK2\' THEN COALESCE(prize_amount, 0) * 1000 ELSE COALESCE(prize_amount, 0) END) as total')
                ->value('total') ?? 0,
        ];
        $total['total_net_win'] = $total['total_payout_amt'] - $total['total_bet_amt'];

        return view('admin.player.report_index', compact('reportDetail', 'total', 'player'));
    }
     ...
     * For Agent role: return the authenticated user (they are the agent)
     * For Owner role: return null (Owner doesn't create players directly)
     */
    private function getAgent()
    {
        $user = Auth::user();
        
        // If user is an Agent, they are the agent themselves
        if ($user->hasRole(self::AGENT_ROLE)) {
            return $user;
        }
        
        // Owner doesn't create players directly, so return null
        // This will fall back to Auth::user() in the calling methods
        return null;
    }
}
