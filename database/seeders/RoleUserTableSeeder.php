    ...
    public function run(): void
    {
        try {
            ...
            foreach (self::ROLE_IDS as $userType => $roleId) {
                ...
                if ($users->isEmpty()) {
                    $roleName = self::ROLE_NAMES[$userType] ?? "Type {$userType}";
                    Log::info("No users found for {$roleName} (type: {$userType}). This is normal if the role is new or unused.");
